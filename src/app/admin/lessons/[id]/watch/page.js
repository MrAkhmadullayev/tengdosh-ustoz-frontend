...
export default function WatchLessonPage() {
	...
	const { id } = useParams() || { id: '4582' } // id bo'lmaganda crash bermasligi uchun default
	...
	const handleSendMessage = e => {
		...
		setMessages([
			...messages,
			{
				id: Date.now(),
				user: 'Siz',
				isMentor: true, // yuboruvchini mentor (ajralib turishi uchun) deb olamiz
				message: chatMessage,
		...
		<div className='max-w-7xl mx-auto pb-4 w-full h-[calc(100vh-5rem)] flex flex-col'>
			...
			<div className='flex items-center gap-4 bg-card p-4 rounded-xl border shadow-sm mb-4 shrink-0'>
				<Button
					variant='ghost'
					size='icon'
					onClick={() => router.back()} // Orqaga qaytish uchun router.back qulayroq
				...
				<div className='flex-1 min-w-0'>
					...
					<p className='text-muted-foreground text-sm mt-0.5 truncate'>
						Mentor: Javohir To'rayev • Dars ID: {id}
					</p>
				</div>
			</div>

			{/* ASOSIY QISM (Video va Chat bir-biriga moslashgan) */}
			<div className='flex-1 flex flex-col lg:flex-row gap-4 min-h-0'>
				{/* VIDEO PLAYER SECTION (Asosiy maydon) */}
				<div className='lg:flex-1 h-full flex flex-col min-h-[300px] lg:min-h-0 relative'>
					<Card className='shadow-sm overflow-hidden border-muted flex-1 bg-black flex flex-col'>
						<div className='flex-1 w-full flex flex-col items-center justify-center text-zinc-500 relative group'>
							...
							<p className='text-sm font-medium'>Translatsiya kutilmoqda...</p>
							...
							<div className='absolute top-4 left-4 flex gap-2'>
								...
								>
									<Users className='w-3 h-3' /> 45
								</Badge>
							</div>
						</div>

						{/* Videoning pastki boshqaruv paneli (Control bar) uchun joy */}
						<div className='h-12 bg-zinc-900/80 border-t border-zinc-800 shrink-0'></div>
					</Card>
				</div>

				{/* RIGHT SIDEBAR - ATTENDEES & CHAT (Aniq height bilan yopilgan) */}
					...
					<Card className='shadow-sm border-muted h-full flex flex-col overflow-hidden'>
						<Tabs defaultValue='chat' className='flex flex-col h-full w-full'>
							...
							>
								<div className='flex-1 overflow-y-auto px-4 pb-4 hide-scrollbar'>
									<div className='space-y-4 pt-2'>
										{messages.map(msg => (
											<div
												key={msg.id}
												className={`flex flex-col ${msg.isMentor ? 'items-end' : 'items-start'}`}
											>
												<div
													className={`flex items-baseline gap-2 mb-1 ${msg.isMentor ? 'flex-row-reverse' : ''}`}
												>
													<span
												...
												<div
													className={`px-3 py-2.5 rounded-2xl max-w-[90%] text-[13px] leading-relaxed break-words ${msg.isMentor ? 'bg-primary text-primary-foreground rounded-tr-sm' : 'bg-muted rounded-tl-sm'}`}
												>
													{msg.message}
												</div>
											</div>
								...
								{/* Xabar yuborish inputi (Doim pastda qotib turadi) */}
