...
export default function ViewTalabaPage() {
		...
		<div className='max-w-5xl mx-auto space-y-6 pb-12'>
			...
			<div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
				<div className='flex items-center gap-4'>
					...
					<div>
						...
						<p className='text-muted-foreground text-sm font-medium'>
							Tizimdagi ID: <span className='text-primary'>{talaba.id}</span>
						</p>
					</div>
				</div>
				<div className='flex w-full sm:w-auto gap-2'>
					<Button
						...
						onClick={() => router.push(`/admin/students/${talaba.id}/message`)}
					>
						<MessageSquare className='h-4 w-4' /> Xabar yozish
					</Button>
					<Button
						className='w-full sm:w-auto gap-2 bg-primary'
						onClick={() => router.push(`/admin/students/${talaba.id}/edit`)}
					>
			...
			<div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
					...
					<Card className='border-none shadow-md bg-card overflow-hidden'>
						...
						<CardContent className='pt-0 flex flex-col items-center px-6 relative'>
							...
							<Badge className='bg-green-500/10 text-green-600 hover:bg-green-500/20 border-none font-medium mb-2 px-3 flex items-center gap-1.5'>
								<CheckCircle2 className='h-3.5 w-3.5' /> Faol Talaba
							</Badge>

							<h2 className='text-2xl font-bold text-center tracking-tight'>
								{talaba.firstName} {talaba.lastName}
							</h2>
							...
							<div className='flex items-center justify-center gap-6 mt-6 w-full pb-6 border-b border-muted'>
								<div className='flex flex-col items-center'>
									<p className='font-bold text-lg'>{talaba.followers}</p>
									<p className='text-xs text-muted-foreground flex items-center gap-1 font-medium'>
										<Heart className='h-3 w-3' /> Obunachi
									</p>
								</div>
								<div className='flex flex-col items-center'>
									<p className='font-bold text-lg'>{talaba.studentsCount}</p>
									<p className='text-xs text-muted-foreground flex items-center gap-1 font-medium'>
							...
							<div className='w-full space-y-4 py-6'>
								<div className='flex items-center gap-3 text-sm'>
									...
									<div className='flex-1'>
										...
										<p className='font-medium'>{talaba.phone}</p>
									</div>
								</div>
								<div className='flex items-center gap-3 text-sm'>
									<div className='bg-muted p-2 rounded-md shrink-0'>
									...
									<div className='flex-1'>
										...
										<p className='font-medium'>{talaba.experience}</p>
									</div>
								</div>
								<div className='flex items-center gap-3 text-sm'>
									<div className='bg-muted p-2 rounded-md shrink-0'>
									...
									<div className='flex-1'>
										...
										<p className='font-medium'>{talaba.totalLessons} soat</p>
									</div>
								</div>
							</div>
						</CardContent>
				...
				<div className='lg:col-span-2 space-y-6'>
					...
					<Card className='shadow-sm border-muted'>
						<CardHeader>
							<CardTitle className='text-lg'>Talaba haqida</CardTitle>
						</CardHeader>
						<CardContent>
					...
					<Card className='shadow-sm border-muted'>
						<CardHeader>
							<CardTitle className='text-lg flex items-center gap-2'>
								<Clock className='h-5 w-5 text-primary' /> Bo'sh vaqtlari
							</CardTitle>
						</CardHeader>
						<CardContent>
							<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3'>
								{talaba.schedule.map((slot, idx) => (
									<div
										key={idx}
							...
							<p className='text-xs text-muted-foreground mt-4 italic'>
								* Talaba jadvalini individual darslar uchun oldindan kelishish
			...
			<div className='mt-8 max-w-5xl mx-auto w-full'>
				<h3 className='text-xl font-bold mb-4 flex items-center gap-2'>
					<PlayCircle className='h-5 w-5 text-primary' /> Talaba qatnashgan
					darslar
				</h3>
